...
#df_arm = pd.read_csv('sensor_csv_kobayashi_arm_mean.csv')
#df_neck = pd.read_csv('sensor_csv_kobayashi_neck_mean.csv')
df_waist = pd.read_csv('accel3_waist_mean_5.csv')

df_picked = df_waist.loc[:, ['x_mean']]
df_picked_y = df_waist.loc[:, ['y_mean']]
df_picked_z = df_waist.loc[:, ['z_mean']]
...
#Norm calculate
norm = np.sqrt(df_waist['x_mean'].values * df_waist['x_mean'].values + df_waist['y_mean'].values * df_waist['y_mean'].values + df_waist['z_mean'].values * df_waist['z_mean'].values)
...
change_x_df = pd.DataFrame(change_x, columns=['change point_x_waist'])
...
abnor_score_df = pd.DataFrame(abnor_score, columns=['abnormal_x_waist'])
data_and_abnor = pd.concat([df_waist.loc[:, ['x_mean']], abnor_score_df], axis=1)
...
change_y_df = pd.DataFrame(change_y, columns=['change point_y_waist'])
...
abnor_score_df_y = pd.DataFrame(abnor_score_y, columns=['abnormal_y_waist'])
data_and_abnor_y = pd.concat([df_waist.loc[:, ['y_mean']], abnor_score_df_y], axis=1)
#data_and_abnor = pd.concat([df.loc[:, ['instant', 'cnt']], abnor_score_df, df_abnor_check], axis=1)
...
change_z_df = pd.DataFrame(change_z, columns=['change point_z_waist'])
...
abnor_score_df_z = pd.DataFrame(abnor_score_z, columns=['abnormal_z_waist'])
data_and_abnor_z = pd.concat([df_waist.loc[:, ['z_mean']], abnor_score_df_z], axis=1)
#data_and_abnor = pd.concat([df.loc[:, ['instant', 'cnt']], abnor_score_df, df_abnor_check], axis=1)
...
change_norm_df = pd.DataFrame(change_norm, columns=['change point_norm_waist'])
...
abnor_score_df_norm = pd.DataFrame(abnor_score_norm, columns=['abnormal_norm_waist'])
...
data_and_abnor_complete.to_csv("CSV_files_variation/data_and_abnor_waist"+ now +".csv", index=False)

change_complete = pd.concat([change_x_df, change_y_df, change_z_df, change_norm_df], axis=1)
change_complete.to_csv("CSV_files_variation/change_point_waist"+ now +".csv", index=False)
...
plt.savefig("PNG_files_variation/abnormalscore_waist"+ now +".png")
